...
class Component {
    ...
    public:
           ...
           // Assume that the rest of the values are filled. (Front-end validation) 
            ...
            this->mqtt.setCallback(
                [this, networkTopic, mqttTopic, dataTopic](char* msgTopic, byte* data, unsigned int length) {
                    ...
                    if (topicMessage.equals(networkTopic)) Component::handleMqttMessageBody("/network.json", data, length);
                    if (topicMessage.equals(mqttTopic)) Component::handleMqttMessageBody("/mqtt.json", data, length);
        ...
        void publishDweb08Data() {
            ...
            for (unsigned int i = 0; i < 4; i++) {
                float temp = sensors.getTempCByIndex(i);
                if (temp != DEVICE_DISCONNECTED_C) doc["temps"].add(temp);
            }
